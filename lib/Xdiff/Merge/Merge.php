...
namespace PeclPolyfill\Xdiff\Merge;
 ...
 */
final class Merge extends AbstractMergeBase
{
     ...
     * {@inheritdoc}
     */
    public function merge(string $base, string $remote, string $local) : mixed
    {
        ...
        $conflicts = [];
        $merged = Merge::mergeHunks($baseLines, $remoteHunks, $localHunks, $conflicts);
        $merged = implode("", $merged);

        if (!empty($conflicts)) {
            return $conflicts;
            //throw new \Exception('A merge conflict has occurred.');
            //throw new \Exception('A merge conflict has occurred.', $conflicts, $merged);
        }
